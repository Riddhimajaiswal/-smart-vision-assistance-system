...
├── Hardware Demo Videos and photos.../ # Demo photos + video
├── SMART VISION ASSISTANCE SYSTEM FOR VISUALLY IMPAIRED (3).pdf
└── final report.pdf
```

All commands below are run from inside the `Smart Vision Assistant/` folder.
...
Detailed project reports are included at the repository root: `SMART VISION ASSISTANCE SYSTEM FOR VISUALLY IMPAIRED (3).pdf` and `final report.pdf`.
